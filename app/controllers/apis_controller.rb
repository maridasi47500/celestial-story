class ApisController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @api.update(api_params)
        format.html { redirect_to @api, notice: "Api was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to apis_path, notice: "Api was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_api
      @api = Api.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def api_params
      params.expect(api: [ :name, :link, :description, :automationpage_id ])
    end
end
